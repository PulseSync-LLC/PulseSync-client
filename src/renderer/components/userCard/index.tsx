        ...
        const updateStatus = () => {
            if (user.status === 'online' && user.currentTrack?.status === 'playing') {
                ...
                setTimeout(() => {
                    ...
                    setStatusUser(prevStatus => (prevStatus.text === 'Слушает' ? fullStatus : { text: 'Слушает', detail: '' }))
            ...
            >
                ...
                <div className={styles.userStatusInfo}>
                    <div className={`${styles.statusText} ${isStatusVisible ? styles.fadeIn : styles.fadeOut}`}>
                        {statusInfo.text}
                        {statusInfo.detail && <span className={styles.statusDetail}> — {statusInfo.detail}</span>}
                    </div>
                </div>
            </button>
        </div>
