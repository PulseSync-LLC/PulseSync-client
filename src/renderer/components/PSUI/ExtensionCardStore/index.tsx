...
const SpinnerIcon = () => (
    <svg className={[st.download_icon, st.spinner].join(' ')} width={20} height={20} viewBox="0 0 24 24" fill="none" aria-hidden="true">
        ...
        {}
        <path d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 0112 4.417v3.013a5 5 0 00-4 4.883H6v2.981z" className={st.spinner_path} fill="currentColor" />
    </svg>
);
...
const TypeBadge: React.FC<{ type: ExtensionType }> = ({ type }) => {
    ...
    let typeClass = '';
    switch (type) {
        case 'css': text = 'Скрипт CSS'; typeClass = st.badge_css; break;
        case 'js': text = 'Скрипт JS'; typeClass = st.badge_js; break;
        case 'both': text = 'CSS + JS'; typeClass = st.badge_both; break;
        default: return null;
    }
        ...
        <article className={rootClassName} style={backgroundStyle}>
            ...
            >
                ...
                {buttonState === 'downloading' && (
                    <div className={st.download_progress_bar} style={{ width: `${progress}%` }} />
                )}
                ...
                <span className={st.download_content}>
                    ...
                    <span className={st.download_count}>
                        {buttonText === 'Скачать' ? downloads || 'N/A' : buttonText}
                    </span>
                </span>
            </button>
        </article>
