...
import {
  AddonConfig, Item, ButtonItem, SliderItem, ColorItem, FileItem,
  SelectorItem, TextItem, ButtonAction, Section
} from './types'
...
const blankItem = (type: Item['type']): Item => {
  switch (type) {
    case 'button':
      return { __k: uid('k_') as any, id: uid('btn_'), name: 'Новая кнопка', description: '', type: 'button', bool: false, defaultParameter: false } as any
    case 'slider':
      return { __k: uid('k_') as any, id: uid('sld_'), name: 'Новый слайдер', description: '', type: 'slider', min: 0, max: 100, step: 1, value: 0, defaultParameter: 0 } as any
    case 'color':
      return { __k: uid('k_') as any, id: uid('col_'), name: 'Новый цвет', description: '', type: 'color', input: '#FFFFFFFF', defaultParameter: '#FFFFFFFF' } as any
    case 'file':
      return { __k: uid('k_') as any, id: uid('file_'), name: 'Новый файл', description: '', type: 'file', filePath: '', defaultParameter: { filePath: '' } } as any
    case 'selector':
      return { __k: uid('k_') as any, id: uid('sel_'), name: 'Новый селектор', description: '', type: 'selector', selected: 1, options: { '1': { event: 'opt_1', name: 'Опция 1' } }, defaultParameter: 1 } as any
    case 'text':
    default:
      return { __k: uid('k_') as any, id: uid('txt_'), name: 'Новый текстовый блок', description: '', type: 'text', buttons: [{ __k: uid('kb_') as any, id: uid('t1_'), name: 'Строка', text: '', defaultParameter: '' }] as any } as any
  }
}

type AddMenuState =
  | { open: false }
  | { open: true; x: number; y: number; dir: 'down' | 'up'; onPick: (t: Item['type']) => void }
...
const ConfigurationSettingsEdit: React.FC<Props> = ({ configData, onChange, save, filePreviewSrc, ...rest }) => {
  ...
  const isDirty = useMemo(
    () => JSON.stringify(stripInternal(cfg)) !== lastSavedSnapRef.current,
    [cfg, savedTick]
  )
  ...
  const openAddMenu = (e: React.MouseEvent, onPick: (t: Item['type']) => void) => {
    ...
    const MENU_ITEM_H = 30, MENU_VPAD = 12, MENU_BORDER = 2, MENU_W = 260
    const MENU_H = typeList.length * MENU_ITEM_H + MENU_VPAD + MENU_BORDER
    const gap = 6
    let x = rect.left, y = rect.bottom + gap, dir: 'down' | 'up' = 'down'
  ...
  useEffect(() => {
    ...
    const escape = (ev: KeyboardEvent) => { if (ev.key === 'Escape') setAddMenu({ open: false }) }
  ...
  const setDragMeta = (e: React.DragEvent, payload: string) => {
    ...
    try { e.dataTransfer.setDragImage(e.currentTarget as HTMLElement, 8, 8) } catch {}
  }
  ...
  const isDirtyEditor = (item: Item, base?: Item): boolean => {
    ...
    switch (item.type) {
      case 'button': return item.defaultParameter !== (base as ButtonItem).defaultParameter
      case 'slider': {
        const b = base as SliderItem
        return item.min !== b.min || item.max !== b.max || item.step !== b.step || item.defaultParameter !== b.defaultParameter
      }
      case 'color': return item.defaultParameter !== (base as ColorItem).defaultParameter
      case 'file': return (item.defaultParameter?.filePath ?? '') !== ((base as FileItem).defaultParameter?.filePath ?? '')
      case 'selector': {
  ...
  const deleteSection = (si: number) =>
    setConfig(produce(cfg, d => { d.sections.splice(si, 1) }))
  ...
  const updateItem = (si: number, ii: number, patch: Partial<Item>) =>
    setConfig(produce(cfg, d => { Object.assign(d.sections[si].items[ii], patch) }))

  const addItemAt = (si: number, index: number, type: Item['type']) =>
    setConfig(produce(cfg, d => { d.sections[si].items.splice(index, 0, blankItem(type)) }))
  ...
  const duplicateItem = (si: number, ii: number) =>
    setConfig(produce(cfg, d => {
      const clone = structuredClone(d.sections[si].items[ii]); (clone as any).__k = uid('k_')
      d.sections[si].items.splice(ii + 1, 0, clone)
    }))

  const deleteItem = (si: number, ii: number) =>
    setConfig(produce(cfg, d => { d.sections[si].items.splice(ii, 1) }))
  ...
  const addTextButton = (si: number, ii: number) =>
    setConfig(produce(cfg, d => {
      ;(d.sections[si].items[ii] as any as TextItem).buttons.push({
        __k: uid('kb_') as any, id: uid('tb_'), name: 'Новая строка', text: '', defaultParameter: '',
      } as any)
    }))

  const removeTextButton = (si: number, ii: number, bi: number) =>
    setConfig(produce(cfg, d => { ;(d.sections[si].items[ii] as TextItem).buttons.splice(bi, 1) }))

  const addSelectorOption = (si: number, ii: number) =>
    setConfig(produce(cfg, d => {
      const it = d.sections[si].items[ii] as SelectorItem
      let nextKey = '1'
      const nums = Object.keys(it.options).map(k => parseInt(k, 10)).filter(x => !isNaN(x))
  ...
  const removeSelectorOption = (si: number, ii: number, key: string) =>
    setConfig(produce(cfg, d => { delete (d.sections[si].items[ii] as SelectorItem).options[key] }))
                  ...
                  <TextInput
                    ...
                    onChange={v =>
                      setConfig(produce(cfg, d => {
                        ...
                        const val = obj[k]; delete obj[k]; obj[String(v || k)] = val
                      }))
                    }
                  />
                  <TextInput
                    ...
                    label="name"
                    value={o.name}
                    onChange={v => setConfig(produce(cfg, d => { ;(d.sections[si].items[ii] as SelectorItem).options[k].name = v }))}
                  />
                  <TextInput
                    name={`opt_evt_${k}`}
                    label="event"
                    value={o.event}
                    onChange={v => setConfig(produce(cfg, d => { ;(d.sections[si].items[ii] as SelectorItem).options[k].event = v }))}
                  />
          ...
          <>
            ...
            {it.buttons.map((b, bi) => (
              <div key={(b as any).__k} className={css.list}>
                <div className={css.inlineGrid}>
                  <TextInput name={`btn_name_${b.id}`} label="Название" value={b.name} onChange={v => updateTextButton(si, ii, bi, { name: v })} />
                  <TextInput name={`btn_id_${b.id}`} label="ID" value={b.id} onChange={v => updateTextButton(si, ii, bi, { id: v })} />
                </div>
                <div className={css.rowWithDelete}>
                  <TextInput name={`btn_def_${b.id}`} label="Default" value={b.defaultParameter ?? ''} onChange={v => updateTextButton(si, ii, bi, { defaultParameter: v })} />
                  <button className={clsx(css.iconBtn, css.dangerBtn)} title="Удалить строку" onClick={() => removeTextButton(si, ii, bi)}>
                    <MdDelete />
                  </button>
                </div>
              </div>
      ...
      >
        <div className={css.itemHeader}>
          ...
          <div className={css.metaGrid}>
            <TextInput name={`item_name_${item.id}`} label="Название" value={item.name} onChange={v => updateItem(si, ii, { name: v } as Partial<Item>)} />
            <TextInput name={`item_id_${item.id}`} label="ID" value={item.id} onChange={v => updateItem(si, ii, { id: v } as Partial<Item>)} />
          </div>

          <div className={css.tools}>
            <button className={css.iconBtn} title="Добавить элемент после"
              onClick={e => openAddMenu(e, (t: Item['type']) => { setAddMenu({ open: false }); addItemAt(si, ii + 1, t) })}>
        ...
        <TextInput name={`item_desc_${item.id}`} label="Описание" value={item.description ?? ''} onChange={v => updateItem(si, ii, { description: v } as Partial<Item>)} />
    ...
    <div ref={rootRef} className={css.root}>
      <div className={css.topBar}>
        ...
        <div className={css.rightBtns}>
          ...
          <button className={css.iconBtn} onClick={() => {
              const map: Record<number, boolean> = {}; cfg.sections.forEach((_, i) => (map[i] = true)); setCollapsed(map)
            }} title="Свернуть все">
            <MdUnfoldLess />
          </button>
        </div>
        ...
        return (
          ...
          >
            <div className={css.sectionHeader}>
              <span className={css.handle} title="Перетащить секцию" draggable onDragStart={onSectionDragStart(si)} onDragEnd={onDragEnd}>
                <MdDragIndicator />
              </span>
              <TextInput
                name={`sec_title_${si}`}
                ...
                value={s.title}
                onChange={v => setConfig(produce(cfg, d => { d.sections[si].title = v }))}
              />
              <div className={css.sectionTools}>
                <button className={css.iconBtn} title="Добавить элемент"
                  onClick={e => openAddMenu(e, (t: Item['type']) => { setAddMenu({ open: false }); addItemEnd(si, t) })}>
                  <MdAdd />
                </button>
                <button className={css.iconBtn} title="Дублировать секцию" onClick={() => duplicateSection(si)}>
                  <MdContentCopy />
                </button>
                <button className={css.iconBtn} title={isCollapsed ? 'Развернуть' : 'Свернуть'} onClick={() => setCollapsed(c => ({ ...c, [si]: !c[si] }))}>
            ...
            {!isCollapsed && (
              <>
                ...
                <div className={css.sectionFooter}>
                  <button className={css.addBtn} onClick={e => openAddMenu(e, t => { setAddMenu({ open: false }); addItemEnd(si, t) })}>
      ...
      {addMenu.open && (
        <div className={clsx(css.addMenu, addMenu.dir === 'up' && css.addMenuUp)} style={{ left: addMenu.x, top: addMenu.y }}>
          {typeList.map(t => (
            <button key={t} className={css.addMenuItem} onClick={() => { addMenu.onPick(t) }}>
              + {t}
            </button>
          ))}
        </div>
      ...
      <ChangesBar open={isDirty} text="Аккуратнее, вы не сохранили изменения!" onReset={() => setCfg(ensureStableKeys(structuredClone(baselineRef.current)))}
                  onSave={async () => {
