...
const ThemeInfo: React.FC<Props> = ({
  ...
  const getAssetUrl = (file: string) =>
    `http://127.0.0.1:${config.MAIN_PORT}/addon_file?name=${encodeURIComponent(
      addon.name,
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (didAcquire) {
        ...
        if (currentBannerKeyRef.current === key)
          currentBannerKeyRef.current = null
      }
    }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (didAcquire) {
        ...
        if (currentLogoKeyRef.current === key)
          currentLogoKeyRef.current = null
      }
    }
  ...
  return (
    <>
      <div
        className={s.themeInfo}
        style={{ backgroundImage: `url(${bannerUrl})` }}
      >
        <div className={s.content}>
          <div
            className={s.libraryLogo}
            onClick={() =>
              nav(`/themes/${encodeURIComponent(addon.directoryName)}`)
            }
          >
            {logoUrl ? (
        ...
        <div className={s.actions} ref={actionsRef}>
          <Button
            className={`${s.toggleButton} ${
              isEnabled ? s.enabledState : s.disabledState
            }`}
