...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    apolloClient
      .query({
        ...
        variables: { name: username, page: 1, pageSize: 50, search: '', sortOptions: [] },
  ...
  const renderTabTitle = () => {
    if (activeTab === 'profile')  return (<><MdPersonOutline size={34} /><span>Профиль {username}</span></>)
    if (activeTab === 'friends')  return (<><MdPeopleOutline size={34} /><span>Друзья {username}</span></>)
    if (activeTab === 'settings') return (<><MdSettings size={34} /><span>Настройки профиля</span></>)
    return null
  }
        ...
        <div className={pageStyles.container}>
          <div className={pageStyles.main_container}>
            <Scrollbar className={styles.scrollArea} classNameInner={styles.scrollAreaInner}>
              ...
              {/* Если вернёшь табы-кнопки — можно отрисовать тут, по аналогии с другими страницами */}
