...
const SETTINGS_KEYS = [
    ...
    'modSavePath',
] as const satisfies readonly (keyof DesktopSettingsPatch)[]
...
const applySettingsPatch = (patch: DesktopSettingsPatch): void => {
    for (const key of SETTINGS_KEYS) {
        ...
        if (value !== undefined) {
            ipcRenderer.send(MainEvents.ELECTRON_STORE_SET, `settings.${key}`, value)
        }
    }
...
const createPulseSyncDesktopApi = (): PulseSyncDesktopApi => ({
    ...
    system: {
        ...
        openAppDirectory: () =>
            ipcRenderer.send(MainEvents.OPEN_PATH, {
                action: 'appPath',
            }),

        openObsWidgetDirectory: () =>
            ipcRenderer.send(MainEvents.OPEN_PATH, {
                action: 'obsWidgetPath',
            }),

        openApplicationsDirectory: () =>
            ipcRenderer.send(MainEvents.OPEN_PATH, {
    ...
    logs: {
        rendererError: text =>
            ipcRenderer.send(MainEvents.RENDERER_LOG, {
                error: true,
                text,
            }),
