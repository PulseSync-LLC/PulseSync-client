...
export const handlePatcherEvents = (window: BrowserWindow): void => {
    ipcMain.on('update-app-asar', async (event, { version, link, checksum }) => {
        try {
            ...
            if (isRunning) {
                event.reply('update-message', { message: 'Закрытие Яндекс Музыки...' });
    ...
    ipcMain.on('remove-mod', async (event) => {
        try {
            ...
            event.reply('remove-mod-failure', { success: false, error: error.message });
        }
    });
};
...
const checkModCompatibility = async (modVersion: string, yandexMusicVersion: string): Promise<boolean> => {
    try {
        ...
        writer.on('finish', async () => {
            ...
            if (checksum) {
                ...
                if (hex !== checksum) {
                    ...
                    event.reply('update-failure', { success: false, error: 'Checksum не совпадает' });
                    return;
                }
            }
