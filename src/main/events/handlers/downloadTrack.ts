...
export async function downloadTrack(event: IpcMainEvent, val: {
    url: string,
    track: Track,
    askSavePath: boolean
}) {
    ...
    const fileExtension = val.track.downloadInfo.codec
        ...
        .replace(/(.*)-mp4/, "$1");
    const defaultFilepath = path.join(downloadDir, `${artists2string(val.track?.artists)} — ${val.track?.title}.`
        ...
        response.on('end', async () => {
            try {
                ...
                if (val.track.coverUri) {
                    ...
                    const coverUrl = 'https://' + val.track.coverUri.replace('%%', '400x400');
